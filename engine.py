...
@torch.no_grad()
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wandb_evaluator, epoch : int, num_batches : int):
    model.eval()
    criterion.eval()

    metric_logger = utils.MetricLogger(prefix='test', epoch=epoch, num_batches=num_batches, delimiter="  ")
    metric_logger.add_meter('class_error', utils.SmoothedValue(window_size=1, fmt='{value:.2f}'))
    header = 'Test:'
